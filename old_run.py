...
def general_modify_temp_lisp_file_to_run(orginal_lisp_file, experiment_folder_path):
    ...
    substitutions = {
        ...
        'script_configure_model': 'idyom:idyom {} \'({}) \'({}) :models :{} :pretraining-ids \'({}) :k {} :detail 3 :output-path \"{}\" :overwrite t'.format(
            TESTID, TARGETVIEWPOINTS, SOURCEVIEWPOINTS, MODELTYPE, TRAINID, RESAMPLENUMBER, DATAOUTPUT),
    }
    ...
    def change_substitution():
        if MODELTYPE == 'stm':
            ...
            substitutions[
                'script_configure_model'] = 'idyom:idyom {} \'({}) \'({}) :models :{} :k {} :detail 3 :output-path \"{}\" :overwrite t'.format(
                TESTID, TARGETVIEWPOINTS, SOURCEVIEWPOINTS, MODELTYPE, RESAMPLENUMBER, DATAOUTPUT)
...
lispfile_to_run = general_modify_temp_lisp_file_to_run(orginal_lisp_file=lispfile_to_read,
                                                       experiment_folder_path=this_experiment_folder)
...
def RunJupyterExperiment(configurations):
    ...
    lispfile_to_run = modify_temp_lisp_file_to_run(orginal_lisp_file=lispfile_to_read,
                                                   experiment_folder_path=this_experiment_folder)
    runLisp(lispfile_to_run)
