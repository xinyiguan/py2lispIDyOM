...
def plot_surprise_across_time(ax, surprise_across_time, title=None):
    ...
    ax.set_xticks(np.arange(*(x_extent + [1])), minor=True)
    ax.set_xlabel(title)
    ax.imshow(surprise_across_time.reshape(1, -1), cmap='Greys', interpolation='gaussian', origin='lower',
              extent=x_extent + [0, 1])
    return ax
...
def make_comparison_figure_from_index(song_index, all_song_dict, pitch_range, output_path):
    ...
    ground_truth_pitch_distribution_from_song_dict = get_pitch_distribution_from_sequence(
        ground_truth_pitch_sequence_from_song_dict, pitch_range)
    ...
    figure_comparison.suptitle(
        'IDyOM Pitch prediction vs ground truth ' + '(' + experiment_name + ')' + '\n\nMelody: ' + melody_name + '\n\n')
    ...
    ax_prediction = plot_pitch_distribution(ax_prediction, predicted_pitch_distribution, title='Pitch Prediction',
                                            pitch_range=pitch_range)
    ax_surprise = plot_surprise_across_time(ax_surprise, surprise,
                                            title='Surprisal across time (darker = more surprise)')
    ax_surprise.yaxis.set_tick_params(labelleft=False)

    ax_ground_truth = figure_comparison.add_subplot(122)
    ax_ground_truth = plot_pitch_distribution(ax_ground_truth, ground_truth_pitch_distribution_from_song_dict,
                                              title='Ground Truth ', pitch_range=pitch_range)
...
def make_comparison_figure_from_history_folder(selected_experiment_history_folder):
    ...
    for i in range(num_of_songs_in_dict):
        ...
        make_comparison_figure_from_index(i, all_song_dict, pitch_range,
                                          output_path=selected_experiment_history_folder + 'viz_pitch_prediction_comparison/')
